...
- The `GITHUB_TOKEN` env var (now provisioned by mise via gopass, see below) also covers
...
- Provider config: `dot_config/opencode/opencode.json.tmpl`. Non-default providers:
  `mistral`, `opencode`, `opencode-go`, `zai-coding-plan`, `lemonade` (local,
